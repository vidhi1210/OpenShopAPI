...
<div id="navigation" class="categorymenu">
	<script type="text/html" id="menuTemplate">
		...
		{#foreach $T.categories as cat}	
			<li class=""><a href="productSearchResult.jsp?refine=cgid={$T.cat.name}">{$T.cat.name}</a>
				<ul class="sf-menu sf-js-enabled">
					{#foreach $T.cat.categories as subcat}	
						<li><a href="productSearchResult.jsp?refine=cgid={$T.cat.name}-{$T.subcat.name}">{$T.subcat.name}</a></li>
					{#/for}
				</ul>
			</li>
		{#/for}
